...
-- Question #1: Pull monthly trends for gsearch sessions and orders so that we can showcase the growth there?
...
-- Question #3: While we’re on Gsearch, could you dive into nonbrand, and pull
...
SELECT 
	YEAR(website_sessions.created_at) AS yr,
	MONTH(website_sessions.created_at) AS mo,
	COUNT(DISTINCT CASE WHEN device_type = 'mobile' THEN website_sessions.website_session_id ELSE NULL END) AS mobile_orders,
    COUNT(DISTINCT CASE WHEN device_type = 'mobile' THEN orders.order_id ELSE NULL END) AS mobile_orders,
    COUNT(DISTINCT CASE WHEN device_type = 'desktop' THEN website_sessions.website_session_id ELSE NULL END) AS desktop_orders,
...
-- Question #4: Can you pull monthly trends for Gsearch, alongside monthly trends for each of our other channels?
...
-- Question #5: Could you pull session to order conversion rates, by month ?
...
-- Question #6: For the gsearch lander test, please estimate the revenue that test earned us Hint: Look at the increase in CVR
...
-- Question #7.	For the landing page test you analyzed previously, it would be great to show a full conversion funnel 
...
-- Question #8 	I’d love for you to quantify the impact of our billing test, as well. Please analyze the lift generated 
...
GROUP BY 1
;
-- $22.83 revenue per billing page 
